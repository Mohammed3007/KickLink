...
// ─── WAITLIST CONFIRM ──────────────────────────────────────────
function ScreenWaitConfirm({ params, store, nav }) {
...
// ─── WAITLIST POSITION ─────────────────────────────────────────
function ScreenWaitPos({ params, store, nav }) {
  const g = klGame(params.id);
  const r = store.regs[g.id] || {};
  const pos = r.waitPos || 2;
  return (
    <div style={{ paddingBottom: 40 }}>
      <KLHeader title="Waitlist" onBack={nav.pop} />
      <div style={{ padding: '4px 16px 0' }}>
        {params.joined && <KLCard style={{ marginBottom: 12, background: KL.blueBg, boxShadow: 'none' }}>
        ...
        <KLCard style={{ marginBottom: 12 }}>
          ...
          <div style={{ background: KL.blueBg, borderRadius: 14, padding: '18px 16px', textAlign: 'center' }}>
            ...
            <div style={{ fontFamily: KL_FONT, fontSize: 12.5, color: KL.blue, opacity: 0.85, marginTop: 10 }}>{pos - 1} {pos - 1 === 1 ? 'person' : 'people'} ahead of you · {g.waitlistCount} waiting total</div>
          </div>
        </KLCard>
        ...
        <KLBtn variant="danger" onClick={() => { nav.setStore((s) => { const regs = { ...s.regs }; delete regs[g.id]; return { ...s, regs }; }); nav.reset('home'); nav.toast('Left the waitlist'); }}>Leave waitlist</KLBtn>
      </div>
    </div>
  );
}
...
function ScreenSpotOffer({ params, store, nav }) {
  const g = klGame(params.id);
        ...
        <div style={{ padding: '4px 16px 0' }}>
          ...
          <div style={{ borderRadius: 20, padding: '22px 18px', marginBottom: 14, textAlign: 'center', background: expired ? KL.grayBg : KL.orangeBg }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', gap: 7, background: '#fff', padding: '6px 14px', borderRadius: 999, marginBottom: 14 }}>
              ...
              <span style={{ fontFamily: KL_FONT, fontSize: 13, fontWeight: 700, color: expired ? KL.gray : KL.orange }}>{expired ? 'OFFER EXPIRED' : 'A SPOT IS YOURS — FOR NOW'}</span>
            </div>
            <div style={{ fontFamily: KL_FONT, fontSize: 13, color: KL.ink2, marginBottom: 4 }}>{expired ? 'This offer has passed to the next player' : 'Accept and pay within'}</div>
            {!expired && <KLCountdownPill seconds={secs} big />}
            <div style={{ fontFamily: KL_FONT, fontSize: 13, color: KL.ink3, marginTop: 6 }}>{expired ? '' : 'or it goes to the next person in line'}</div>
          </div>
          ...
          <KLCard style={{ marginBottom: 12, background: KL.violetTint, boxShadow: 'none' }}>
            <div style={{ display: 'flex', gap: 9 }}>
              ...
              <div style={{ fontFamily: KL_FONT, fontSize: 13.5, color: KL.violet, opacity: 0.92, lineHeight: 1.45 }}>{g.policy} Accepting confirms your spot immediately after payment.</div>
            </div>
          </KLCard>
        </div>
      </div>
        ...
        {expired
          ...
          : <>
              ...
              <div style={{ textAlign: 'center', marginTop: 10 }}>
                <span onClick={() => { nav.setStore((s) => ({ ...s, offer: { ...s.offer, status: 'declined' } })); nav.reset('home'); nav.toast('Spot offered to the next player'); }} style={{ fontFamily: KL_FONT, fontSize: 15, fontWeight: 600, color: KL.red, cursor: 'pointer' }}>Decline spot</span>
              </div>
            </>}
      </KLFooter>
    </KLFlow>
...
// ─── OFFER MY SPOT ─────────────────────────────────────────────
function ScreenOfferSpot({ params, store, nav }) {
  const g = klGame(params.id);
  const [mode, setMode] = React.useState('waitlist');
  const [player, setPlayer] = React.useState(null);
  const eligible = klOrg(g.org) && g.confirmed ? [] : [];
    ...
    <KLFlow>
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', minHeight: 0 }}>
        ...
        <div style={{ padding: '8px 16px 0' }}>
          ...
          <div style={{ marginBottom: 12 }}>
            <KLSegment value={mode} onChange={setMode} options={[{ id: 'waitlist', label: 'The waitlist' }, { id: 'specific', label: 'A specific player' }]} />
          </div>
            ...
            <KLCard style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', gap: 10 }}>
                ...
                <div>
                  ...
                  <div style={{ fontFamily: KL_FONT, fontSize: 13.5, color: KL.ink2, marginTop: 3, lineHeight: 1.45 }}>{g.waitlist.length} {g.waitlist.length === 1 ? 'person is' : 'people are'} waiting. They’ll be offered your spot in order and must pay to claim it.</div>
                </div>
              </div>
            </KLCard>
          ) : (
          ...
          <KLCard style={{ marginBottom: 12, background: KL.greenBg, boxShadow: 'none' }}>
            <div style={{ display: 'flex', gap: 9 }}>
              ...
              <div>
                ...
                <div style={{ fontFamily: KL_FONT, fontSize: 13.5, color: KL.green, opacity: 0.92, marginTop: 3, lineHeight: 1.5 }}>You stay registered until a replacement pays. Only then is your spot transferred and your ${g.price}.00 refunded{g.transferApproval ? '. The organizer approves transfers.' : '.'}</div>
              </div>
            </div>
          </KLCard>
        </div>
      ...
      <KLFooter note={g.transferApproval ? 'The organizer will review this request.' : undefined}>
        <KLBtn icon="arrowSwap" disabled={mode === 'specific' && !player} onClick={() => {
          nav.setStore((s) => ({ ...s, regs: { ...s.regs, [g.id]: { ...s.regs[g.id], reg: 'offered' } }, transfer: { gameId: g.id, mode, player, status: g.transferApproval ? 'approval' : 'offered' } }));
          nav.replace('transferStatus', { id: g.id });
        }}>{mode === 'specific' ? `Offer to ${player ? player.split(' ')[0] : 'player'}` : 'Offer to waitlist'}</KLBtn>
      </KLFooter>
    </KLFlow>
  );
}
...
// ─── TRANSFER STATUS ───────────────────────────────────────────
function ScreenTransferStatus({ params, store, nav }) {
  const g = klGame(params.id);
  const t = store.transfer || { status: 'offered', mode: 'waitlist' };
  const steps = [
    { key: 'offered', label: 'Spot offered', sub: t.mode === 'specific' ? `Sent to ${t.player}` : 'Sent to the waitlist', done: true },
    ...
    { key: 'accept', label: 'Replacement accepts & pays', sub: 'In progress', active: t.status !== 'approval' },
    { key: 'done', label: 'Transfer complete', sub: `You’re refunded $${g.price}.00`, done: false },
  ];
  return (
    <KLFlow>
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', minHeight: 0 }}>
              ...
              <div key={s.key} style={{ display: 'flex', gap: 12, paddingBottom: i < steps.length - 1 ? 4 : 0 }}>
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
                  <div style={{ width: 22, height: 22, borderRadius: '50%', background: s.done ? KL.green : s.active ? KL.orange : KL.fill, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                    {s.done ? <KLIcon name="check" size={13} color="#fff" sw={3} /> : s.active ? <span style={{ width: 7, height: 7, borderRadius: 7, background: '#fff' }} /> : <span style={{ width: 6, height: 6, borderRadius: 6, background: KL.ink3 }} />}
                  </div>
      ...
      <KLFooter note="Changed your mind? You can cancel the offer while it’s pending.">
        <KLBtn variant="danger" onClick={() => { nav.setStore((s) => ({ ...s, regs: { ...s.regs, [g.id]: { ...s.regs[g.id], reg: 'confirmed' } }, transfer: null })); nav.reset('home'); nav.toast('Transfer offer cancelled'); }}>Cancel offer</KLBtn>
      </KLFooter>
    </KLFlow>
  );
}
...
// ─── SHARE EVENT ───────────────────────────────────────────────
function ScreenShareEvent({ params, nav }) {
  const g = klGame(params.id);
  const rows = [
  ...
  return (
    <div style={{ paddingBottom: 40 }}>
      ...
      <div style={{ padding: '8px 16px 0' }}>
        ...
        <KLCard pad={0}>
          {rows.map((r, i) => (
            <KLRow key={i} icon={r[0]} iconBg={r[1]} title={r[2]} sub={r[3]} last={i === rows.length - 1} onClick={() => nav.toast(i === 0 ? 'Opening WhatsApp…' : 'Copied to clipboard')} chevron={false} right={<KLIcon name={i < 1 ? 'share' : 'copy'} size={18} color={KL.ink3} />} />
          ))}
        </KLCard>
      </div>
    </div>
...
function ScreenReceipts({ store, nav }) {
        ...
        <KLCard pad={0}>
          {list.map((p, i) => (
            <KLRow key={p.id} last={i === list.length - 1} chevron onClick={() => nav.toast('Receipt opened')}
              ...
              right={<div style={{ textAlign: 'right', marginRight: 4 }}>
                <div style={{ fontFamily: KL_FONT, fontSize: 15, fontWeight: 700, color: p.status === 'refunded' ? KL.gray : KL.ink, textDecoration: p.status === 'refunded' ? 'line-through' : 'none' }}>${p.amount.toFixed(2)}</div>
      ...
      <KLCard pad={0} style={{ marginBottom: 16 }}>
        {items.map((it, i) => (
          <KLRow key={it.k} chevron={false} last={i === items.length - 1} title={it.t} sub={it.s}
            right={it.locked ? <span style={{ fontFamily: KL_FONT, fontSize: 12, color: KL.ink3, display: 'flex', alignItems: 'center', gap: 4 }}><KLIcon name="lock" size={13} color={KL.ink3} /> Always on</span> : <KLToggle on={on[it.k]} onToggle={() => toggle(it.k)} />} />
        ))}
      </KLCard>
    </>
  );
    ...
    <div style={{ paddingBottom: 40 }}>
      ...
      <div style={{ padding: '4px 16px 0' }}>
        <Group title="Critical (transactional)" items={[
          { k: 'pay', t: 'Payment & receipts', s: 'Confirmations, refunds', locked: true },
...
// ─── ORGANIZER APPLY ───────────────────────────────────────────
function ScreenOrganizerApply({ nav }) {
  return (
    <KLFlow>
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', minHeight: 0 }}>
        ...
        <div style={{ padding: '8px 16px 0' }}>
          <div style={{ textAlign: 'center', padding: '6px 0 18px' }}>
            <div style={{ width: 64, height: 64, borderRadius: 20, background: `linear-gradient(135deg, ${KL.violet}, #8A57E6)`, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 12px' }}><KLIcon name="org" size={32} color="#fff" /></div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <KLField label="CLUB OR ORGANIZATION NAME" value="" placeholder="e.g. Westside Sunday League" icon="org" />
            <KLField label="CITY" value="Ottawa, ON" icon="pin" />
            <KLField label="PHONE" value="" placeholder="+1 (613) 000-0000" icon="person" />
            <div style={{ display: 'flex', gap: 10 }}>
              <div style={{ flex: 1 }}><KLField label="GAMES / MONTH" value="" placeholder="4–8" /></div>
              <div style={{ flex: 1 }}><KLField label="PLAYERS" value="" placeholder="~40" /></div>
            </div>
            <div>
              <div style={{ fontFamily: KL_FONT, fontSize: 13, color: KL.ink3, fontWeight: 600, margin: '0 0 6px 4px' }}>WHAT DO YOU ORGANIZE?</div>
              <div style={{ background: '#fff', border: `1px solid ${KL.line}`, borderRadius: 13, padding: '12px 14px', minHeight: 70, fontFamily: KL_FONT, fontSize: 14, color: KL.ink3 }}>Weekly 7-a-side pickup at Brewer Park, plus occasional indoor futsal in winter…</div>
            </div>
            <div style={{ display: 'flex', gap: 9, alignItems: 'center', padding: '4px 4px' }}>
              <div style={{ width: 22, height: 22, borderRadius: 6, background: KL.violet, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><KLIcon name="check" size={14} color="#fff" sw={3} /></div>
...
// ─── ORGANIZER STATUS ──────────────────────────────────────────
function ScreenOrganizerStatus({ nav }) {
  const steps = [
    { label: 'Application submitted', sub: 'Today · 9:41 AM', done: true },
    { label: 'Under review', sub: 'A KickLink admin is reviewing', active: true },
    { label: 'Identity & payout setup', sub: 'After approval' },
    { label: 'Organizer tools unlocked', sub: 'Create your first game' },
  ];
  return (
    <div style={{ paddingBottom: 40 }}>
      ...
      <div style={{ padding: '8px 16px 0' }}>
        <div style={{ textAlign: 'center', padding: '6px 0 18px' }}>
          <div style={{ width: 80, height: 80, borderRadius: '50%', background: KL.amberBg, display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px' }}><KLIcon name="clock" size={40} color={KL.amber} /></div>
...
Object.assign(window, {
  ...
  ScreenTransferStatus, ScreenShareEvent, ScreenReceipts, ScreenNotifPrefs,
  ScreenPrivacy, ScreenOrganizerApply, ScreenOrganizerStatus,
});
