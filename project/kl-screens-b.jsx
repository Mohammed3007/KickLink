...
function KLFooter({ children, note }) {
  return (
    <div style={{
...
// fixed-height column: middle scrolls, footer pinned as a normal bottom child
function KLFlow({ children }) {
  return <div style={{ height: '100%', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>{children}</div>;
}

// ─── Capacity bar ──────────────────────────────────────────────
function KLCapacity({ filled, capacity, accent }) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 7 }}>
        ...
        <span style={{ fontFamily: KL_FONT, fontSize: 13.5, fontWeight: 700, color: left <= 3 && left > 0 ? KL.orange : left === 0 ? KL.red : KL.green }}>{left > 0 ? `${left} left` : 'Full'}</span>
      </div>
...
// ─── GAME DETAILS ──────────────────────────────────────────────
function ScreenGameDetails({ params, store, nav }) {
  ...
  return (
    <KLFlow>
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', minHeight: 0 }}>
        <KLHeader title={g.title} onBack={nav.pop} trailing={<button onClick={() => nav.push('shareEvent', { id: g.id })} style={{ border: 'none', background: 'none', cursor: 'pointer', padding: 6 }}><KLIcon name="share" size={21} color={KL.violet} /></button>} />
          ...
          <KLCard style={{ marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginTop: 14 }} onClick={() => nav.push('participants', { id: g.id })}>
              <div style={{ display: 'flex' }}>
                {preview.map((p, i) => (
                  <div key={i} style={{ marginLeft: i ? -10 : 0, borderRadius: 30, boxShadow: '0 0 0 2.5px #fff' }}><KLAvatar name={p.name} size={30} /></div>
                ))}
              </div>
...
// ─── PARTICIPANTS ──────────────────────────────────────────────
function ScreenParticipants({ params, store, nav }) {
  const g = klGame(params.id);
  return (
...
// ─── JOIN REVIEW (confirmation before pay) ─────────────────────
function ScreenJoinReview({ params, store, nav }) {
  const g = klGame(params.id);
          ...
          {!free && (
            <KLCard pad={0} style={{ marginBottom: 12 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '13px 16px' }}>
                <div style={{ width: 32, height: 32, borderRadius: 9, background: KL.fill, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><KLIcon name="person2" size={19} color={KL.ink2} /></div>
                <div style={{ flex: 1 }}>
...
function KLPriceLine({ label, value, bold }) {
  return (
...
// ─── CHECKOUT ──────────────────────────────────────────────────
function ScreenCheckout({ params, store, nav }) {
  const g = klGame(params.id);
  const total = params.total != null ? params.total : g.price;
  const [method, setMethod] = React.useState('applepay');
  const [pay, setPay] = React.useState(null); // null | 'sheet' | 'processing'

  const confirmPaid = () => {
    nav.setStore((s) => ({
      ...s,
      regs: { ...s.regs, [g.id]: { reg: 'confirmed', pay: 'paid', method: method === 'applepay' ? 'Apple Pay' : 'Visa ·· 4242' } },
      payments: [{ id: 'np' + g.id, title: g.title, org: klOrg(g.org).name, amount: total, method: method === 'applepay' ? 'Apple Pay' : 'Visa ·· 4242', date: 'Jun 13, 2026', status: 'paid' }, ...s.payments],
    }));
...
// Apple Pay sheet (Face ID → done)
function KLApplePaySheet({ total, game, onCancel, onDone }) {
    ...
    <div style={{ position: 'absolute', inset: 0, zIndex: 96, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      ...
      <div style={{ position: 'relative', background: '#fff', borderRadius: '14px 14px 0 0', padding: '18px 18px 34px', animation: 'klSlideUp .3s cubic-bezier(.32,.72,0,1)', boxShadow: '0 -8px 40px rgba(0,0,0,0.25)' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
          <svg width="20" height="24" viewBox="0 0 22 26"><path d="M15 6.3c.8-1 1.4-2.4 1.2-3.8-1.2.05-2.6.8-3.5 1.8-.8.9-1.5 2.3-1.3 3.6 1.3.1 2.7-.7 3.6-1.6z" fill="#000"/><path d="M16.2 8.2c-1.9-.1-3.5 1.1-4.4 1.1-.9 0-2.3-1-3.8-1C6 8.3 4.2 9.4 3.2 11.2c-2 3.5-.5 8.7 1.4 11.5.9 1.4 2 2.9 3.4 2.9 1.4-.05 1.9-.9 3.5-.9 1.6 0 2.1.9 3.6.86 1.5-.03 2.4-1.4 3.3-2.8 1-1.6 1.5-3.1 1.5-3.2-.03-.02-2.9-1.1-2.9-4.4 0-2.7 2.2-4 2.3-4.1-1.3-1.9-3.3-2.1-4-2.1z" fill="#000"/></svg>
        ...
        <div style={{ border: `1px solid ${KL.line}`, borderRadius: 12, padding: '12px 14px', marginBottom: 12 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <div style={{ width: 40, height: 26, borderRadius: 5, background: '#1A1F71', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><span style={{ color: '#fff', fontSize: 10, fontWeight: 800, fontStyle: 'italic' }}>VISA</span></div>
            <div style={{ flex: 1 }}>
        ...
        <div style={{ display: 'flex', justifycontent: 'space-between', justifyContent: 'space-between', padding: '4px 2px 16px', borderTop: `0.5px solid ${KL.line}`, marginTop: 6 }}>
        ...
        <div style={{ textAlign: 'center', minHeight: 60 }}>
          {stage === 'confirm' && (
            <button onClick={() => setStage('auth')} style={{ width: '100%', height: 50, borderRadius: 12, border: 'none', background: '#000', color: '#fff', fontFamily: KL_FONT, fontSize: 16, fontWeight: 600, cursor: 'pointer' }}>Confirm with Face ID</button>
          )}
          {stage === 'auth' && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 44, height: 44, borderRadius: 11, border: `2.5px solid ${KL.ink}`, position: 'relative', animation: 'klPulse 1s ease-in-out infinite' }}>
          ...
          {stage === 'done' && (
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8 }}>
              <div style={{ width: 46, height: 46, borderRadius: '50%', background: KL.green, display: 'flex', alignItems: 'center', justifyContent: 'center', animation: 'klPop .3s ease' }}><KLIcon name="check" size={26} color="#fff" sw={3} /></div>
...
// ─── SUCCESS ───────────────────────────────────────────────────
function ScreenSuccess({ params, store, nav }) {
  const g = klGame(params.id);
  const confirmed = !params.later;
  return (
    <div style={{ minHeight: '100%', display: 'flex', flexDirection: 'column', background: confirmed ? KL.bg : KL.bg }}>
...
function ScreenReg({ params, store, nav }) {
  const g = klGame(params.id);
  const r = store.regs[g.id] || {};
  ...
  return (
    <KLFlow>
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', minHeight: 0 }}>
        <KLHeader title="Your registration" onBack={nav.pop} trailing={<button onClick={() => nav.push('shareEvent', { id: g.id })} style={{ border: 'none', background: 'none', cursor: 'pointer', padding: 6 }}><KLIcon name="share" size={21} color={KL.violet} /></button>} />
        <div style={{ padding: '4px 16px 0' }}>
          {/* status banner */}
          <div style={{ borderRadius: 18, padding: '16px 18px', marginBottom: 12, background: provisional ? KL.amberBg : KL.greenBg }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div style={{ flex: 1 }}>
                ...
                <div style={{ fontFamily: KL_FONT, fontSize: 13, color: provisional ? KL.amber : KL.green, opacity: 0.9 }}>{provisional ? `Pay $${g.price}.00 within ${g.payDeadlineHrs}h to keep it` : (r.pay === 'free' ? 'Free event' : `Paid · ${r.method || 'Apple Pay'}`)}</div>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 7, marginTop: 12 }}>
              <KLStatus status={r.reg} small />
              <KLStatus status={r.pay === 'free' ? 'free' : r.pay} small icon={r.pay !== 'free'} />
            </div>
          </div>
...
// ─── CANCEL ────────────────────────────────────────────────────
function ScreenCancelReg({ params, store, nav }) {
  const g = klGame(params.id);
  const r = store.regs[g.id] || {};
    ...
    <KLFlow>
      <div style={{ flex: 1, overflowY: 'auto', overflowX: 'hidden', minHeight: 0 }}>
        ...
        <div style={{ padding: '8px 16px 0' }}>
          ...
          <KLCard pad={0} style={{ marginBottom: 12 }}>
            <KLRow chevron={false} title="Tip: offer your spot instead" sub="Get a faster refund if someone takes it" right={<KLIcon name="arrowSwap" size={20} color={KL.orange} />} onClick={() => nav.replace('offerSpot', { id: g.id })} last />
          </KLCard>
        </div>
      </div>
      <KLFooter note="This can’t be undone. Refund follows the organizer’s policy.">
        ...
        }}>{refundable ? `Cancel & refund $${g.price}.00` : 'Cancel registration'}</KLBtn>
        <div style={{ textAlign: 'center', marginTop: 10 }}><span onClick={nav.pop} style={{ fontFamily: KL_FONT, fontSize: 15, fontWeight: 600, color: KL.violet, cursor: 'pointer' }}>Keep my spot</span></div>
      </KLFooter>
    </KLFlow>
  );
}
...
Object.assign(window, {
  KLFooter, KLFlow, KLCapacity, KLPriceLine, KLToggle, KLPayMethod, KLApplePaySheet,
