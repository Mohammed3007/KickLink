// KickLink — screens A: tab roots (Home, Games, Clubs, Org detail, Notifications, Profile)

// ─── Shared: date chip ─────────────────────────────────────────
function KLDateChip({ game, tone }) {
...
// ─── Shared: game list card ────────────────────────────────────
function KLGameCard({ game, store, nav, statusOverride }) {
  const r = store.regs[game.id] || {};
...
// ─── HOME ──────────────────────────────────────────────────────
function ScreenHome({ store, nav }) {
  const offerActive = store.offer && store.offer.status === 'pending' && (store.regs.g2 || {}).reg !== 'confirmed';
  const g4 = store.regs.g4 || {};
  return (
    <div style={{ paddingBottom: 104 }}>
      <KLHeader large title="Hi, Daniel" subtitle="Sunday, June 13" trailing={
      ...
      <div style={{ padding: '4px 16px 0' }}>
        ...
        {store.regs.g3 && <KLGameCard game={klGame('g3')} store={store} nav={nav} />}
        <KLGameCard game={klGame('g4')} store={store} nav={nav} />

        {/* Discover */}
        <div style={{ height: 6 }} />
        <KLSectionLabel action="Clubs" onAction={() => nav.switchTab('orgs')}>Open games in your clubs</KLSectionLabel>
        <KLGameCard game={klGame('g1')} store={store} nav={nav} />
...
// Countdown pill (visual; ticks via store)
function KLCountdownPill({ seconds, big }) {
...
// ─── GAMES ─────────────────────────────────────────────────────
function ScreenGames({ store, nav }) {
  ...
  return (
    <div style={{ paddingBottom: 104 }}>
      ...
      <div style={{ padding: '0 16px' }}>
        {tab === 'upcoming' && (mine.length ? mine.map((g) => <KLGameCard key={g.id} game={g} store={store} nav={nav} />) : <KLEmpty icon="calendar" title="No upcoming games" body="Join an open game in one of your clubs." />)}
        {tab === 'open' && open.map((g) => <KLGameCard key={g.id} game={g} store={store} nav={nav} />)}
        {tab === 'past' && past.map((h) => (
...
function KLEmpty({ icon, title, body, cta, onCta }) {
  return (
...
// ─── ORG DETAIL ────────────────────────────────────────────────
function ScreenOrgDetail({ params, store, nav }) {
  const o = klOrg(params.id);
  const games = KL_GAMES.filter((g) => g.org === o.id);
  return (
    <div style={{ paddingBottom: 40 }}>
      <KLHeader title={o.name} onBack={nav.pop} trailing={<button onClick={() => nav.toast('Invite link copied')} style={{ border: 'none', background: 'none', cursor: 'pointer', padding: 6 }}><KLIcon name="share" size={21} color={KL.violet} /></button>} />
      {/* cover */}
      ...
      <div style={{ padding: '0 20px', marginTop: -28 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 4 }}>
          ...
          <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, fontFamily: KL_FONT, fontSize: 13.5, color: KL.ink3 }}><KLIcon name="lock" size={13} color={KL.ink3} /> Private · {o.handle}</span>
        </div>
      ...
      <div style={{ padding: '20px 16px 0' }}>
        ...
        {games.map((g) => <KLGameCard key={g.id} game={g} store={store} nav={nav} />)}
      </div>
    </div>
  );
}
...
// ─── ANNOUNCEMENTS ─────────────────────────────────────────────
function ScreenAnnouncements({ params, nav }) {
    ...
    <div style={{ paddingBottom: 40 }}>
      ...
      <div style={{ padding: '8px 18px 0' }}>
        <div style={{ textAlign: 'center', padding: '14px 0 22px' }}>
          ...
          <div style={{ fontFamily: KL_FONT, fontSize: 17, color: KL.ink2, lineHeight: 1.45 }}>KickLink clubs are invite-only.<br />Enter a club ID or open an invite link.</div>
        </div>
        <KLField label="CLUB ID" value="" placeholder="e.g. WSL-4471" icon="org" />
        <div style={{ height: 12 }} />
        ...
        <div style={{ fontFamily: KL_FONT, fontSize: 12.5, color: KL.ink3, textAlign: 'center', marginTop: 14, lineHeight: 1.5 }}>You'll see the club before joining. Some clubs require organizer approval.</div>
      </div>
    </div>
  );
}
...
// ─── NOTIFICATIONS ─────────────────────────────────────────────
function ScreenNotifs({ store, nav }) {
  const iconFor = { offer: ['ball', KL.orange], reminder: ['clock', KL.amber], announce: ['bell', KL.violet], waitlist: ['arrowSwap', KL.blue], receipt: ['receipt', KL.green] };
  const go = (n) => {
              ...
              <div style={{ display: 'flex', gap: 12 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    ...
                    {n.unread && <span style={{ width: 8, height: 8, borderRadius: 8, background: KL.violet }} />}
                  </div>
...
function KL_STATUS_BG(c) {
  const map = { [KL.orange]: KL.orangeBg, [KL.amber]: KL.amberBg, [KL.violet]: KL.violetTint, [KL.blue]: KL.blueBg, [KL.green]: KL.greenBg };
...
Object.assign(window, {
  KLDateChip, KLGameCard, KLCountdownPill, KLEmpty,
  ScreenHome, ScreenGames, ScreenOrgs, ScreenOrgDetail, ScreenAnnouncements,
  ScreenJoinOrg, ScreenNotifs, ScreenProfile, KL_STATUS_BG,
});
