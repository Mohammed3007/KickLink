...
// reliability indicator (private)
const KL_RELIABILITY = { label: 'Good standing', tone: 'good', games: 23, cancels: 1, noshows: 0 };
...
// Participant pools
function klPool(names) {
  return names.map((n) => ({
    name: n, initials: klInitials(n), color: klAvatarColor(n),
  }));
}
...
const KL_GAMES = [
  ...
  },
  {
    ...
    waitlistCount: 0,
    model: 'later', // join now pay later
    payDeadlineHrs: 36,
...
function klGame(id) { return KL_GAMES.find((g) => g.id === id); }
function klOrg(id) { return KL_ORGS.find((o) => o.id === id); }

// Announcements
const KL_ANNOUNCEMENTS = [
  { id: 'a1', org: 'westside', author: 'Marcus (Organizer)', time: '2h ago',
    title: 'Light + dark this Sunday', body: 'Please bring both shirts — we’ll split teams on the spot. Kickoff 8:00 sharp, gates lock at 8:05.' },
  { id: 'a2', org: 'westside', author: 'Marcus (Organizer)', time: 'Yesterday',
    title: '3 spots left for Sunday', body: 'Filling up fast. Grab your spot before Friday or you’ll be on the waitlist.' },
];

// Notifications (mutable at runtime via store)
const KL_NOTIFS = [
...
Object.assign(window, {
  KL_ME, KL_RELIABILITY, KL_ORGS, KL_GAMES, KL_ANNOUNCEMENTS, KL_NOTIFS,
  KL_PAYMENTS, KL_HISTORY, klGame, klOrg, klAvatarColor, klInitials, klPool,
});
